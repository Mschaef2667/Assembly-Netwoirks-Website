...
- Pre-Beta Legal Checklist (urgent - Legal and Protection): 1) Trademark C3 Method and Assembly AI at USPTO.gov before public launch. 2) Add Terms of Service and Privacy Policy pages to the app at /terms and /privacy. 3) Add Proprietary and Confidential -- Assembly AI footer to all PDF exports (Strategic Plan, DCP Map, Insights, Interview Guide). 4) Ensure GitHub repo is private. 5) Have all beta clients sign NDA before access. 6) Add beta agreement to onboarding flow.

- In-app Beta Feedback Widget (planned - high priority pre-beta): Add a floating feedback button on every page (bottom right corner, orange, with speech bubble icon). Clicking opens a small modal with three options: 1) Thumbs Up/Down for the current step or page, 2) Report an Issue (captures current URL, step ID, and text description), 3) Share an Idea (feature suggestion). All feedback saves to a beta_feedback Supabase table with columns: id, org_id, user_id, page_url, step_id, type (thumbs_up/thumbs_down/issue/idea), message, created_at. Admin can view all feedback in the Administration section. Build as a client component that wraps the entire app layout.

- Beta Client Onboarding Process (planned): 1) NDA and beta agreement signed before access granted. 2) 30-minute onboarding call walking through the platform. 3) Getting Started guide (PDF or in-app). 4) Week 1: Phase 1 + Intelligence (surveys and DCP Map). 5) Week 2-3: Journey Steps 4-30. 6) Week 4: Strategic Plan generation and review call. 7) Structured feedback collection using the beta feedback widget. Build a Beta Clients section in the Administration page showing each beta client workspace, their progress through the journey, and their feedback submissions.
