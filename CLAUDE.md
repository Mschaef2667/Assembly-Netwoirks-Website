...
- [x] Engagement Plan rename from Strategic Plan (Phase 6 section; migration 20260824000000 + normalizeSection; "Action Plan" now reserved for the future Idea Filter implementation roadmap)
...
- Future State Strategic Plan (planned - high priority): A second strategic deliverable that lives alongside the existing Strategic Plan at the end of Journeys. Available only after Gate 1 is approved (DCP Map complete). While Plan 1 (Current State Engagement Plan) is based on Phase 1 data and tells clients what to do now, Plan 2 (Future State Strategic Plan) is based on the Insights module output and tells clients what to build toward over 6-18 months.
  ...
  Delivery: Delivered as a SEPARATE PDF download from the current Strategic Plan. Button labeled Generate Future State Plan appears on the Journeys report page alongside the existing Download PDF and Download Word buttons. Only active when Gate 1 is approved AND Insights have been generated. The Future State Plan focuses on 6-18 month strategic initiatives to strengthen market position based on gaps, opportunities and threats discovered during the journey.

  The Plan 2 inputs should specifically include: 1) Capability Gaps from Steps 13-14 (Critical gaps that need to be resolved), 2) Competitive Opportunities from Step 25 (market gaps nobody is filling), 3) Competitive Threats from Step 20 (what competitors do better), 4) Competitive Retaliation from Step 24 (counter-strategies), 5) Insights module output (internal vs external gaps, brand perception gaps, segment differences). The 6-18 month roadmap should prioritize: closing critical capability gaps first, then pursuing competitive opportunities, then executing retaliation strategies. This gives clients a clear long-term strategic agenda beyond the immediate engagement plan.
...
- Upstream Edit Cascade Notifications (planned - high priority): When a user edits and saves an upstream step, the system should detect which downstream steps depend on it and notify the user. Dependency map: Step 4 affects Steps 5,6,7,8,9,10,11,27. Step 5 affects Step 27. Step 6 affects Steps 10,11,27,30. Step 8 affects Step 10. Step 11 affects Step 28. Step 14 affects Step 28. Step 18 affects Step 29. Step 19 affects Step 30. Implementation: when a step is saved with new content, compare to previous version hash. If changed, look up downstream dependents, show a notification banner listing affected steps with a Refresh Now button. Clicking Refresh regenerates affected steps via auto-apply Copilot. If dismissed, show an amber Outdated badge on affected step cards in the Journey list. This keeps the Strategic Plan internally consistent when upstream content changes.
- Step 22 Copilot JSON parsing bug (known issue): The Generate with Copilot button on Step 22 (Competitive Evaluation) fails to parse the response even though the API returns valid JSON. The response comes back as a non-streaming text fetch with backtick fences. The parseCopilotJson function cannot find the JSON despite multiple fix attempts. Needs a fresh debugging session. Workaround: users can fill in the 8 sections manually for now. The structured editor UI is working correctly -- only the Copilot auto-fill is broken.
- Capability Gap Dashboard Widget (planned): Add a card to the Workspace Dashboard that surfaces the most critical gaps identified in Steps 13 (Critical Success Formulas) and 14 (Core Competencies). Show: number of Critical gaps, number of High gaps, top 2-3 gap names with red/amber badges, and a link to Go to Step 13/14. This keeps capability gaps top of mind as users work through the rest of the journey. The widget should update automatically when Step 13 or 14 assessments are saved. Color coding: red for any Critical gaps present, amber for High gaps only, green when all gaps are Low or None.
- Content Source Transparency (planned - pre-beta): Tag every Copilot-generated content block with its source: Survey-Grounded (derived from actual buyer responses in DCP Map), AI-Inferred (generated by Copilot from web search or general knowledge, needs validation), or Company-Provided (entered manually by user). Display source badges on each step content block. In the Strategic Plan PDF, add footnotes on AI-Inferred sections recommending buyer validation before external presentation. Steps that are primarily Survey-Grounded: 4-9 (from DCP Map), 11 (CVPs grounded in buyer language). Steps that are primarily AI-Inferred: 17-26 (competitive analysis), 22 (evaluation process). Steps that are mixed: 10 (formula), 12-16 (company formulas), 27-30 (strategic messages). This feature protects the credibility of the Strategic Plan and helps users know where to focus validation efforts.
- Textarea spacing display (polish): Numbered list items have blank lines in the data but HTML textareas collapse multiple newlines visually. Consider switching auto-generated content display to a styled read-only div that renders proper paragraph spacing, while keeping the textarea for manual editing. Low priority -- spacing renders correctly in the Strategic Plan PDF.
- Step 3.5 GTM Snapshot Consolidation (planned): Step 3.5 asks for current GTM metrics, sales motion, and biggest challenges but this information should be captured in Step 1 (Company Profile) during initial setup. Plan: 1) Enhance Step 1 to include a GTM Context section with fields for current metrics, sales motion (PLG/sales-led/channel/hybrid), biggest challenges, and current tech stack. 2) Remove Step 3.5 from the journey or repurpose it for something else. 3) Update all Copilot prompts that currently pull from Step 3.5 to pull from the enhanced Step 1 instead. This eliminates redundant data entry and ensures GTM context is available from the start of the journey.
- Next button not activating on page load (known bug): The Next button requires a manual refresh to activate after navigating to a step that already has saved content. The hasContent check evaluates correctly after user interaction but not on initial page load after data is fetched. Fix: trigger hasContent re-evaluation after step_output data loads in useStepContext hook. Apply fix across all step editors -- PainPointStepEditor, ActionPlanEditor, AssessmentStepEditor, AcidTestEditor, CompetitiveEvaluationEditor, DecisionProcessEditor, CompetitorStepEditor.

- Step 38 Opportunity Evaluation (move to Lead Generation): Step 38 is currently a placeholder in Phase 6 but belongs in the Lead Generation module. When Lead Generation is built, move Step 38 there and remove it from the Journey phases entirely. The step defines opportunity scoring criteria against ICP which is a prospecting/qualification tool not a strategic planning step.
- Company Logo Upload (planned - pre-beta polish): Allow users to upload their company logo in workspace settings. The logo should appear: 1) On the workspace dashboard header, 2) On all PDF exports (Strategic Plan, DCP Map, Insights Report, Interview Guide) replacing or alongside the Assembly AI branding, 3) On the public survey page so respondents see the client company logo. Implementation: add a logo_url column to organizations table, build an upload component in workspace settings using Supabase Storage, update all PDF generation functions to include the logo image. This is an important personalization feature for beta clients who will share reports externally.

- Change Password in Settings (planned): Add a Change Password section to user account settings. Fields: Current Password, New Password, Confirm New Password. Call supabase.auth.updateUser with new password on submit. Also add a user Settings page if one does not exist -- accessible from the sidebar user area at the bottom.
