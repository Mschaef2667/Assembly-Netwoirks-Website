...
async function handleSubmit(req: NextRequest): Promise<Response> {
  let body: SubmitBody
  try {
  ...
  const { token, respondentName, respondentTitle, respondentCompany, respondentSize, respondentIndustry, answers } = body
  ...
  const { error: insertError } = await serviceRole
    ...
    .insert({
      ...
      respondent_industry: respondentIndustry || null,
      answers: answers ?? {},
    })
