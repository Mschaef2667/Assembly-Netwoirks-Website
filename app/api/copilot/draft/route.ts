...
Be specific, actionable, and grounded in the prerequisite data. Do not hallucinate facts not present in the context. Draft directly — do not ask the user for more information.${extraContext ? `\n\n${extraContext}` : ''}`
  ...
  // ── Step 16: direct non-streaming call, return parsed JSON ────────────────
  if (stepId === '16') {
    try {
      ...
      try {
        ...
        console.error('[copilot/draft] Step 16 JSON parse failed. Raw response (first 800 chars):', rawText.slice(0, 800))
        return NextResponse.json({ error: 'Copilot returned an invalid response.' }, { status: 502 })
      }
      ...
      console.error(`[copilot/draft] Step 16 Anthropic call failed (HTTP ${status}):`, errMsg)
      try {
        await supabase.from('copilot_run').insert({
          workspace_id: workspaceId,
          step_id: stepId,
      ...
      return NextResponse.json({ error: errMsg }, { status: status >= 400 && status < 600 ? status : 500 })
    }
  }
