...
export default function InsightsPage() {
  ...
  async function downloadDocx() {
    if (downloadingDocx || !insights) return
    ...
    try {
      ...
      const company = orgName || 'Your Company'
      const overall = insights.overall_confidence
      const overallLabel = confLabel(overall)
      ...
      children.push(new Paragraph({
        children: [new TextRun({ text: 'Intelligence Insights Report', bold: true, size: 36 })],
        spacing: { after: 200 },
      }))
      children.push(new Paragraph({
      ...
      const metaRows: [string, string][] = [
        ['Company', company],
        ['Date', today],
        ['Overall Confidence', `${overallLabel} · ${overall}/100`],
      ]
      for (const [label, value] of metaRows) {
        children.push(new Paragraph({
          children: [
      ...
      for (const meta of CATEGORIES) {
        const data = insights.categories[meta.key]
        const confidence = typeof data?.confidence === 'number' ? data.confidence : 0
        const cLabel = confLabel(confidence)
        const cColor = confidence >= 70 ? '16A34A' : confidence >= 40 ? 'D97706' : 'DC2626'
        ...
        children.push(new Paragraph({
          children: [
            new TextRun({ text: `${cLabel} Confidence · ${confidence}/100`, bold: true, size: 22, color: cColor }),
          ],
          spacing: { after: 120 },
        }))

        children.push(new Paragraph({
          children: [new TextRun({ text: meta.description, italics: true, size: 22, color: '6B7280' })],
          spacing: { after: 160 },
        }))

        children.push(new Paragraph({
          children: [new TextRun({ text: 'Insights', bold: true, size: 24, color: '0A1628' })],
        ...
        if (items.length === 0) {
          children.push(new Paragraph({
            children: [new TextRun({ text: 'No insights generated for this category.', italics: true, size: 22, color: '9CA3AF' })],
            spacing: { after: 80 },
          }))
        } else {
          for (const item of items) {
      ...
      const doc = new Document({
        creator: 'Assembly AI',
        title: `${company} — Intelligence Insights Report`,
      ...
      a.href = url
      a.download = `${company.toLowerCase().replace(/\s+/g, '-')}-intelligence-insights.docx`
      a.click()
